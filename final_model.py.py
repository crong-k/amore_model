...
image_size = 0
directory_root = 'gt_data_big/gt_train'
directory_root_test = 'gt_data_big/gt_test'
...
# suffle train data
import random
conbine = list(zip(image_list, label_list ))
random.shuffle(conbine)
image_list_s, label_list_s  = zip(*conbine)

label_binarizer = LabelBinarizer()
y_train = label_binarizer.fit_transform(label_list_s)
pickle.dump(label_binarizer,open('label_transform.pkl', 'wb'))
n_classes = len(label_binarizer.classes_)
print('maybe pkl file order => ',label_binarizer.classes_)

x_train = np.array(image_list_s, dtype=np.float16) / 255.0  #scailing 0~1
...
x_test = np.array(image_list, dtype=np.float16) / 255.0  #scailing 0~1
